<template>
    <div id="my-order">
      ...
      <div class="order-container" v-if="myOrderInfo.length">
        <div class="item" v-for="(item,index) in myOrderInfo">
          <div class="item-top">
            <span class="cinema-name">{{item.cinema_name}}</span>
            <button class="status" v-show="!(new Date().getTime()>new Date(item.show_date).getTime())" @click="cancel(index)" :key="index">退票</button>
            <span class="status" v-show="new Date().getTime()>new Date(item.show_date).getTime()">已完成</span>
          </div>
...
<script>
    import {getOrderByUserId,cancelOrder} from '../../../api'
    ...
    export default {
        ...
        methods:{
          ...
          cancel(index){
            var info = "确认退票";
            console.log(this.myOrderInfo[index].order_id)
            MessageBox.confirm(info,'退票提示').then(async action =>{
              if (action==='confirm'){
                  let json = await cancelOrder(this.myOrderInfo[index].order_id,this.myOrderInfo[index].schedule_id,this.myOrderInfo[index].order_seat_info);
                  if (json.success_code===200){
