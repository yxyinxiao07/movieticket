  ...
  <div id="my-info">
    ...
    <div class="info-container">
      <div class="list">
        ...
        <div
          class="item"
          @click.prevent="$router.push({name:'modify_userphone',params:{userPhone:userPhone}})"
        >
          <span>手机号</span>
          <span class="right">
            {{userPhone?userPhone:'未填写'}}
            <i class="icon-more"></i>
          </span>
        </div>
        <div
          class="item"
          @click.prevent="$router.push({name:'modify_userpwd',params:{userPwd:userPwd}})"
        >
          <span>密码</span>
          <span class="right">
            {{qpassword?qpassword:'未填写'}}
            <i class="icon-more"></i>
          </span>
        </div>
    ...
    updateUserInfo() {
      if (this.jsonData) {
        ...
        this.jsonData.user_name
          ...
          : (this.userName = "");
        this.jsonData.phone
          ? (this.userPhone = this.jsonData.phone)
          : (this.userPhone = "");
    ...
    async loadUserInfo() {
      if (this.$cookies.get("user_id")) {
        ...
        if (json.success_code === 200) {
          ...
          console.log(this.jsonData.password)
          if(this.jsonData.password){
            this.qpassword = this.jsonData.password.replace(/./g,"*")
          }
          this.updateUserInfo();
        } else {
          this.jsonData = {};
