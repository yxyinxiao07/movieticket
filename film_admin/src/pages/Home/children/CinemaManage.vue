<template>
    <div id="cinema-manage">
      ...
      <div class="top">
        <el-col :span="12">
          <el-input placeholder="影院名" v-model="input" class="input-with-select" style="width: 100%">
      ...
      <div>
        ...
        >
          <el-form :label-position="labelPosition" label-width="80px" :model="cinemaInfo" :rules="rules">
            ...
            <el-form-item prop='province' label='所在省'>
                <el-select
                    v-model='cinemaInfo.province'
            ...
            <el-form-item prop='city' label='所在市'>
                <el-select
                    v-model='cinemaInfo.city'
                >
                    <el-option
                        v-for='item in city'
                        :key='item'
...
<script>
    ...
    export default {
        ...
        methods: {
          getCityInfo () {
              this.axios.get('../../../static/json/city.json').then(this.getCityInfoSucc)   //请求本地配置的mock数据
          },
          getCityInfoSucc(res){
              res = res.data
              // console.log(res);
              res.forEach(provinceItem=>{
                ...
                var province = provinceItem.name;
                let cityList = provinceItem.city;
                this.province.push(province);
                cityList.forEach(cityItem=>{
                  let city = cityItem.name;
                    let key = province + '-' + city;
                    // console.log(key)
                    var arr = {};
          ...
          async manageCinemaInfo(){
            ...
            if (!this.cinemaInfo.cinema_name||!this.cinemaInfo.province||!this.cinemaInfo.city||!this.cinemaInfo.cinema_phone||!(/^1[3|4|5|6|7|8][0-9]{9}$/.test(this.cinemaInfo.cinema_phone))||!this.cinemaInfo.specified_address) {
              Message.error('请完成必填内容！');
            } else{
                if (this.dialogTitle==='编辑影院信息') {
                  let json = await updateCinemaInfo(this.cinemaInfo.cinema_id,this.cinemaInfo.cinema_name,this.cinemaInfo.cinema_phone,this.cinemaInfo.province,this.cinemaInfo.city,this.cinemaInfo.specified_address);
                  if (json.success_code===200){
                  ...
                  let json = await addCinemaInfo(this.cinemaInfo.cinema_name,this.cinemaInfo.cinema_phone,this.cinemaInfo.province,this.cinemaInfo.city,this.cinemaInfo.specified_address);
                  if (json.success_code===200){
                    this.loadCurrentPageCinema(this.currentPage,8,this.searchInput);
                    this.dialogFormVisible = false;
