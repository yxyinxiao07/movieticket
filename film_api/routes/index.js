    ...
    conn.query(sqlStr,[phone],(error,result,field)=>{
      if (error){
        ...
        if (result[0]){
          ...
          conn.query(sqlStr, [phone, phone, avatarSrc, password],(error,result,field)=>{
            if (error){
              console.log(error);
        ...
        conn.query(sqlStr, [userName, userId], (error, result, field) => {
          if (error) {
            ...
            if (result[0]) {
              ...
              //更新数据库
              let sqlStr = 'UPDATE t_user SET user_name = ?, password = ? WHERE user_id = ?;';
              conn.query(sqlStr, [userName, userPwd, userId], (error, result, field) => {
                if (error) {
                  res.json({ error_code: 1, message: '补齐信息失败' });
                } else {
                  res.json({ success_code: 200 });
                }
              });
    ...
    conn.query(sqlStr,[name],(error,result,field)=>{
      if (error){
        ...
        if (result[0]){
          ...
          res.json({ error: 1, message:'用户不存在，请输入正确的用户名'});
        }
      }
    })
  }
...
//更新手机号
router.post('/api/updateUserPhone', function (req, res) {
  let {
    userId,
    userPhone,
  } = req.body;
  if (userId) {
    ...
    conn.query(sqlStr, [userId], (error, result, field) => {
      if (error) {
        ...
        conn.query(sqlStr, [userPhone, userId], (error, result, field) => {
          if (error) {
            console.log(error);
          } else {
            result = JSON.parse(JSON.stringify(result));
            if (result[0]) {
              res.json({ error_code: 1, message: '手机号已存在！' });
            } else {
              //更新数据库
              let sqlStr = 'UPDATE t_user SET phone = ? WHERE user_id = ?;';
              conn.query(sqlStr, [userPhone, userId], (error, result, field) => {
                if (error) {
                  res.json({ error_code: 1, message: '更新手机号失败' });
                } else {
                  res.json({ success_code: 200 });
                }
              });
  ...
  if (userId) {
    ...
    conn.query(sqlStr, [userId], (error, result, field) => {
      if (error) {
        ...
        sqlStr = 'SELECT * FROM t_user WHERE password = ? AND user_id <> ? LIMIT 1 ;';
        conn.query(sqlStr, [userPwd, userId], (error, result, field) => {
          if (error) {
            ...
            conn.query(sqlStr, [userPwd, userId], (error, result, field) => {
              if (error) {
                res.json({ error_code: 1, message: '更新密码失败' });
              } else {
                res.json({ success_code: 200 });
              }
            });
...
router.get('/api/getMovieList', function (req, res) {
  ...
  let sqlStr = 'SELECT * FROM t_schedule INNER JOIN t_movie ON t_schedule.movie_id = t_movie.movie_id INNER JOIN t_cinema ON t_schedule.cinema_id = t_cinema.cinema_id AND t_cinema.city = ?;';
  conn.query(sqlStr,[city],(error,result,field)=>{
    if (error){
      console.log(error);
...
router.get('/api/getCinemaList', function (req, res) {
  ...
  let sqlStr = 'SELECT * FROM t_cinema WHERE city = ?;';
  conn.query(sqlStr,[city],(error,result,field)=>{
...
router.get('/api/matchCinemaByName',function (req,res) {
  ...
  let sqlStr = 'SELECT * FROM t_cinema WHERE cinema_name LIKE ? AND t_cinema.city = ?;';
  conn.query(sqlStr,['%'+cinemaName+'%',city],(error,result,field)=>{
    console.log(result)
    if (error){
      console.log(error);
    } else{
...
router.post('/api/cancelOrder', function (req, res) {
  ...
  // conn.query(sqlStr, [oederId], (error, result, field) => {
  //   if (error) {
  //     console.log(error);
  //   } else {
  //     res.json({ success_code: 200 });
...
//注册
router.post('/api/admin/regist', function (req, res) {
  let { name, password, sex, phone, birthday, avatar } = req.body;
  if (!avatar) {
  ...
  conn.query(sqlStr, [name], (error, result, field) => {
    if (error) {
      ...
      result = JSON.parse(JSON.stringify(result));
      if (result[0]) {
        res.json({ error_code: 1, message: '管理员已存在！' });
      } else {
        sqlStr = 'INSERT INTO t_admin(name, password, sex, phone, birthday, avatar) VALUES(?,?,?,?,?,?);';
        conn.query(sqlStr, [name, password, sex, phone, birthday, avatar], (error, result, field) => {
          if (error) {
            console.log(error);
          } else {
            res.json({ success_code: 200 });
        ...
        conn.query(sqlStr,[cinemaName,cinemaId],(error,result,field)=>{
          if (error){
            ...
            if (result[0]){
              ...
              //更新数据库
              let sqlStr = 'UPDATE t_cinema SET cinema_name = ?,cinema_phone = ?,province = ?,city = ?,specified_address = ? WHERE cinema_id = ?;';
              conn.query(sqlStr, [cinemaName, cinemaPhone,province,city,address,cinemaId],(error,result,field)=>{
                if (error){
  ...
  conn.query(sqlStr,[cinemaName],(error,result,field)=>{
    if (error){
      ...
      if (result[0]){
        ...
        let sqlStr = 'INSERT INTO t_cinema(cinema_name,cinema_phone,province,city,specified_address) VALUES(?,?,?,?,?);';
        conn.query(sqlStr, [cinemaName, cinemaPhone, province,city,address],(error,result,field)=>{
          if (error){
            console.log(error);
          } else{
            res.json({success_code:200});
